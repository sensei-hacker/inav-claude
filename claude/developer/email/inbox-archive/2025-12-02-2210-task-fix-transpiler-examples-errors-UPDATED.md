...
Fix THREE related bugs in JavaScript Programming transpiler that are breaking example code:

1. **gpsNumSat → gpsSats** - Outdated property name in GPS example
2. **altitude-stages crash** - Missing null check causes "Cannot read properties of undefined (reading 'targets')"
3. **waypoint.distanceToHome → waypoint.distance** - Wrong property name in waypoint example

All three are user-blocking bugs in example code.
...
## Bug #2: Waypoint Distance Property

### Problem

```
Transpilation Error:
Unknown property 'distanceToHome' in 'waypoint.distanceToHome'.
Available: isWaypointMission, number, action, nextAction, distance, distanceFromPrevious, ...
```

**Occurs when:** Transpiling the "Waypoint Arrival Detection" example

### Root Cause

The example uses `waypoint.distanceToHome` but the correct property is `waypoint.distance` (distance to current waypoint, not home).

### Fix

**File:** `js/transpiler/examples/index.js`

Lines 185, 189: Change `distanceToHome` → `distance`
...
### Step 1: Fix Examples (Bugs #1 and #2)
...
sed -i 's/waypoint\.distanceToHome/waypoint.distance/g' examples/index.js
```
...
### Step 2: Fix Property Checker (Bug #3)
...
### Test Bug #2 (waypoint.distanceToHome → waypoint.distance):

1. JavaScript Programming tab
2. Load "Waypoint Arrival Detection" example from dropdown
3. Click "Transpile"
4. **Expected:** Success (no "Unknown property 'distanceToHome'" error)
...
### Test Bug #3 (altitude-stages crash):
...
**2 files, ~12 lines total:**

1. `js/transpiler/examples/index.js` (4 line changes)
   - Line 120: gpsNumSat → gpsSats
   - Line 124: gpsNumSat → gpsSats
   - Line 185: waypoint.distanceToHome → waypoint.distance
   - Line 189: waypoint.distanceToHome → waypoint.distance
...
- [ ] gpsNumSat replaced with gpsSats in examples (lines 120, 124)
- [ ] waypoint.distanceToHome replaced with waypoint.distance (lines 185, 189)
- [ ] Null checks added to property_access_checker.js
- [ ] "GPS Fix Check" example transpiles successfully
- [ ] "Waypoint Arrival Detection" example transpiles successfully
...
- GPS example was not updated, causing transpilation errors
...
- Correct property is 'waypoint.distance' (distance to waypoint)
