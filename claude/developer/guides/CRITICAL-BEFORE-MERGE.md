...
- **A conflict where one side changed a truthy check's semantics (not just its wording) can leave a test fixture silently testing the wrong thing**: on PR #2732's re-sync, one branch refactored `if (this._connectionId) {...}` into a helper call, while the other branch fixed the same guard from `if (this._connectionId)` to `if (this.hasConnectionId())` (because `0` is a legitimate id, only `null`/`false` mean "disconnected"). Resolving by keeping the refactor and swapping in the new helper was correct for the *production* code, but a test elsewhere in the suite simulated "port disappeared" via `_connectionId = 0` — under the new semantics that's now a *valid* connection, so the test silently stopped exercising the code path it claimed to test (it still ran, just asserted the wrong thing, until the assertion failed). After resolving a conflict that changes what counts as "empty"/"disconnected"/"unset" for a value, `grep` the test suite for that field and re-check any fixture using the old sentinel, not just the production call sites the conflict marker touched.
- **When two sibling PRs fix the same defect class, the surviving *comments* are the risk, not the values**: merging the already-merged PR #11815 into PR #11803 (both from `investigate-shared-tim-dma-request-lines`) conflicted in 18 `target.c` files. The functional side was unambiguous — #11815's `dmavar` values were a strict superset of #11803's in every file, never a contradiction. But #11803 also carried documenting comments of the form `// S6 -- clash with S5, DSHOT does not work`, and #11815 had since *actually fixed* several of those collisions, so a comment that was true when written now asserted a working output was broken. Note this cuts both ways: 3 of the 16 comments lost by taking the base side verbatim were still true and had to be restored, and one needed rewording because the *mechanism* had changed (the stream clash was gone; the shared request line remained). Verify every surviving and every dropped comment against the merged state, not against either parent, and prefer the project's own analysis tooling over reading the source — hand-reasoning 18 targets' DMA topology is exactly where this goes wrong.
- **A merge can silently inherit a hazard the other branch introduced and never documented**: #11815 moved BEEROTORF4's M1 to `dmavar 1`, which put it on DMA2 Stream2 — and both of `TIM8_CH1`'s options are Stream2, so M5 lost DSHOT. A net improvement (it trades a dead output at position 1 for one at position 4) but undocumented anywhere. When merging a branch that touched the same subsystem, diff the *hazard set* before and after the merge (not just the file diff), and document anything the merge newly introduces even when the change came from the other side.
...
- **Pushing a resolved conflict back to a contributor's fork branch needs a remote you don't already have, and permission you should check first**: `git push shota3527 HEAD:their-branch` (as written above) assumes a `shota3527` remote already exists — it usually doesn't. Check `gh api repos/<org>/<repo>/pulls/<N> --jq .maintainer_can_modify` first (true means the PR author allowed maintainer edits); if true, `git remote add <name> git@github.com:<fork-owner>/<repo>.git`, fetch the PR's head branch from it, and push there directly with your own SSH credentials — no fork of your own needed. If `maintainer_can_modify` is false, you can't push to their branch at all; comment on the PR with the fix instead.
- **A bug a bot flags on a dependent PR's diff can actually live in the PR it depends on**: reviewing PR #2725 (which "carries #2710's commits" until #2710 merges), Qodo's finding pointed at `js/mission_3d.js` code that turned out to be entirely #2710's own, unmodified by #2725 — it only showed up in #2725's diff because that file didn't exist yet on the shared base branch. `git log --oneline -1 -- <file>` on the dependent branch shows which PR's commit actually introduced the flagged lines; fix it on that PR's own branch, not the dependent one, so the fix flows through the natural merge rather than needing to be reconciled twice.
...
- [ ] For any whole-file conflict resolved as "just drift", `git grep` the PR's changed symbol/function on the base branch to check whether it moved elsewhere rather than merely changing
- [ ] Ran `inav-code-review` against the resolution diff itself — a clean build only catches a missing import, not a leftover half-converted call site
- [ ] Pushed and watched full CI (all targets, not just a local build) — especially if the branch's last green run predates a large chunk of base-branch history
- [ ] If a conflict changed what counts as "empty"/"disconnected"/"unset" for a value, grepped the test suite for that field and re-checked any fixture using the old sentinel
- [ ] If gating new code behind an `MCU_RAM_SIZE`-style macro, checked `src/test/unit/CMakeLists.txt` for a matching macro definition the unit test needs
- [ ] If part of this merge was delegated to a fork/subagent, verified directly (`git status`, grep for conflict markers, `git log`) rather than trusting the completion notification text
