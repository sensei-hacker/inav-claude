...
- **A subagent's "it crashed" verdict on SITL/MSP is a hypothesis, not a fact** (2026-09-04): a sitl-operator run reported SITL "crashing"/timing out on an MSP request; the firmware was actually fine — the test script itself was querying the wrong MSP field index (misreading a bundled-reply protocol convention). Before accepting a subagent's failure diagnosis on a live-protocol test, reproduce with a minimal hand-crafted raw request/response probe yourself; it's fast and distinguishes "the target is broken" from "the test's model of the protocol is wrong" far more reliably than re-running the same failing test harness.
...
- **Assert a bug's mechanism only after tracing the code that sets the value** (2026-09-15): "MANUAL is continuously on" was asserted as "MANUAL is the default fallback" without reading the firmware; the real cause is `processRx()` force-enabling `MANUAL_MODE` while `!ARMED && areSensorsCalibrating()` (bench passthrough), and Acro (no flight-mode flag) is the true default. Inference from the symptom is not root cause.
- **A UI state's color encodes semantics — confirm it before redefining the condition that drives it** (2026-09-15): light-blue `on` means "the FC reports this mode currently active", so a fix that lights the ACRO banner `on` while MANUAL is also `on` shows two mutually-exclusive modes as simultaneously current — the fix changed the indicator's meaning to "switch-selected" without changing the visual language, which is a new bug, not a fix.
- **A "missing callback" hypothesis needs to be traced through what actually runs synchronously, not just labeled a "race"** (2026-09-16, issue #11721): the initial diagnosis for geozones dropping on save+reboot was "an ack race — `reboot()` might fire before `MSP_EEPROM_WRITE` is acknowledged." Tracing `reboot()` → `GUI.tab_switch_cleanup()` → `mspQueue.flush()` (`js/serial_queue.js`) showed the flush runs *synchronously in the same tick*, so in the common case the queued write is discarded before it's even transmitted — not a rare ack race but a near-guaranteed drop. Both diagnoses point to the same fix (gate `reboot()` on `saveToEeprom()`'s callback), but only the deeper trace explained why the bug went from "worked in 9.0.0" to "reliably reproduces in 9.1.0": a race is intermittent, a synchronous same-tick flush is not. When a fix works, still trace the actual synchronous call chain the "race" runs through — it can turn out to be deterministic, which changes how confidently you can explain the reported symptom's reliability.
