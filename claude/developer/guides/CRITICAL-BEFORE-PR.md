...
- **inav-configurator's SonarCloud check gates on ≤3% duplication on *new* code, and it's a separate required check from Qodo/builds**: a two-test file (a bug repro + a positive-control test with nearly identical setup/execute/assert bodies) failed SonarCloud at 18.3% even though Qodo found zero issues and all platform builds passed. When writing paired/parameterized-style tests (repro + control, or several near-identical cases), factor the shared setup/execute sequence into one helper function up front rather than duplicating it — cheaper than discovering the gate failure after push and needing a follow-up commit.
