#!/usr/bin/env python3
"""
GPS RTH Bug Test - Tests GPS recovery with RTH mode active

This test is designed to reproduce issue #11049:
- Fly away from home
- Activate RTH mode
- Simulate GPS loss
- Check if homeDistance incorrectly becomes 0 after GPS recovery

BOXNAVRTH = 8 (from rc_modes.h)

Usage:
    python3 gps_rth_test.py 5761
"""
...
def send_rc_gps(board, throttle=RC_LOW, aux1=RC_LOW, aux2=RC_LOW, gps_fix=2, gps_sats=12,
    ...
    AUX2 = RTH (channel 6)
    """
    # Send RC - AETR order: Roll, Pitch, Throttle, Yaw, AUX1, AUX2, ...
    channels = [RC_MID, RC_MID, throttle, RC_MID, aux1, aux2, RC_LOW, RC_LOW]
...
def setup_arm_mode(board):
    ...
    payload = [0, BOXARM, 0, 32, 48]  # AUX1, 1700-2100
...
def setup_rth_mode(board):
    ...
    # Mode range: index, boxId, auxChannelIndex (0=AUX1), range_start, range_end
    # AUX2 = channel index 1 (0=AUX1, 1=AUX2)
    # Range 32-48 = 1700-2100 in PWM
    payload = [1, BOXNAVRTH, 1, 32, 48]  # mode_range[1], BOXNAVRTH, AUX2, 1700-2100
    board.send_RAW_msg(MSP_SET_MODE_RANGE, data=payload)
    consume_response(board)
...
def run_rth_gps_test(board, base_lat, base_lon, base_alt, gps_loss_seconds=3.0, log_file=None):
    """Run the GPS RTH bug test scenario with RTH mode active."""
    ...
    def send_now():
        ...
        send_rc_gps(board, throttle=throttle, aux1=aux1, aux2=aux2, gps_fix=gps_fix,
                    gps_sats=gps_sats, gps_lat=current_lat, gps_lon=current_lon,
                    gps_alt=gps_alt, gps_speed=gps_speed)

    log("\n" + "="*60)
    log("GPS RTH Bug Test - Issue #11049")
    log("="*60)
    ...
    log(f"GPS loss duration: {gps_loss_seconds} seconds")
    ...
    send_now()
    log(f"  -> Reached: {dist_before_rth}m from home")

    if dist_before_rth is None or dist_before_rth < 50:
    ...
    # Continue flying (simulating movement) for a second to let RTH activate
    start_time = time.time()
    ...
    send_now()
    log(f"  -> RTH activated. Distance to home: {dist_at_rth}m")

    # Phase 5: GPS loss for specified duration
    log(f"\n[Phase 5] Simulating GPS loss for {gps_loss_seconds} seconds...")
    loss_lat = current_lat
    loss_lon = current_lon

    gps_fix = 0
    gps_sats = 0
    # Keep sending last known position during loss (will be ignored by FC due to fix=0)
    # Actually, set to 0 to be more realistic
    current_lat = 0
    current_lon = 0
    gps_alt = 0
    gps_speed = 0
    ...
    start_time = time.time()
    last_log_time = 0
    while time.time() - start_time < gps_loss_seconds:
        send_now()
        time.sleep(0.02)
        ...
        if elapsed - last_log_time >= 0.5:
            ...
            dist = query_distance_to_home(board)
            send_now()
            log(f"  t={elapsed:.1f}s GPS lost: Distance to home = {dist}m")
            last_log_time = elapsed

    send_now()
    dist_during_loss = query_distance_to_home(board)
    send_now()
    log(f"  -> End of GPS loss. Distance to home: {dist_during_loss}m")

    # Phase 6: GPS recovery
    log("\n[Phase 6] Simulating GPS recovery...")
    gps_fix = 2
    gps_sats = 10
    # Recover at original loss position (plus a bit of drift)
    current_lat = loss_lat + int(20 / METERS_PER_UNIT)  # 20m further
    ...
    while time.time() - start_time < 5.0:
        ...
        if elapsed - last_log_time >= 0.5:
            ...
            dist = query_distance_to_home(board)
            send_now()
            log(f"  t={elapsed:.1f}s GPS recovered: Distance to home = {dist}m")
            last_log_time = elapsed

    send_now()
    dist_after_recovery = query_distance_to_home(board)
    send_now()
    log(f"  -> Final distance to home: {dist_after_recovery}m")
    ...
    log(f"Distance end of GPS loss: {dist_during_loss}m")
...
def main():
    parser = argparse.ArgumentParser(description='GPS RTH Bug Test')
    ...
    parser.add_argument('--gps-loss', type=float, default=3.0, help='GPS loss duration in seconds')
    ...
    if args.log:
        ...
        log_file.write(f"GPS loss duration: {args.gps_loss}s\n")
    ...
    try:
        ...
        with MSPy(device=device, use_tcp=True, loglevel='WARNING') as board:
            ...
            # Run the test
            result = run_rth_gps_test(board, base_lat, base_lon, args.alt,
                                       gps_loss_seconds=args.gps_loss, log_file=log_file)
