#!/usr/bin/env python3
"""
MSP Benchmark Tool for Serial Port Testing

Tests MSP request throughput by sending multiple requests
and measuring response times over a serial port.
"""
...
def send_msp_requests_serial(port_name: str, baudrate: int, commands: List[int], count: int = 10) -> Tuple[float, int, int]:
    """
    ...
    try:
        ...
        read_count = [0]  # Use list for nonlocal mutable
        def receive_responses():
            ...
            while receiving:
                try:
                    ...
                    break
            print(f"[RX] Receiver thread exiting. Total: {read_count[0]} chunks, {len(response_data)} bytes")
        ...
        # Send requests at 120 Hz (120 requests/second)
        request_interval = 1.0 / 120.0  # 8.33ms between requests
        ...
        # On serial, send itself takes ~2.5s, so wait longer
        time.sleep(5.0)
        receiving = False
...
def run_benchmark(port_name: str, baudrate: int, iterations: int = 3):
    """Run benchmark test"""
    ...
    print(f"\n{'='*60}")
    print(f"MSP Benchmark Test (Serial Port)")
    print(f"{'='*60}")
    ...
    for run in range(iterations):
        ...
        try:
            total_time, sent, received = send_msp_requests_serial(
                port_name, baudrate, test_commands, requests_per_iteration
            )
...
def main():
    ...
    try:
        result = run_benchmark(port, baudrate, iterations)
        if result:
            sys.exit(0)
        else:
            sys.exit(1)
